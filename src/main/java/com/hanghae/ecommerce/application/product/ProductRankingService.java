 ...
 * 
 * ## 구현 방식
 * - Redis Sorted Set 사용: Key = "product:ranking", Member = productId, Score = 주문 수량
...
public class ProductRankingService {
    ...
    public List<RankedProduct> getTopRankedProducts(int limit) {
        ...
        // 랭킹 정보와 함께 RankedProduct 객체 생성
        int rank = 1;
        List<RankedProduct> result = new java.util.ArrayList<>();
        
        for (ZSetOperations.TypedTuple<Object> tuple : rankedProducts) {
            if (tuple.getValue() == null) {
                continue;
            }
            
            Long productId = Long.valueOf(tuple.getValue().toString());
            Long orderCount = tuple.getScore() != null ? tuple.getScore().longValue() : 0L;
            
            Product product = productMap.get(productId);
            Stock stock = stockMap.get(productId);
            
            if (product != null && stock != null) {
                result.add(new RankedProduct(rank++, product, stock, orderCount.intValue()));
            }
        }
    ...
    public static class RankedProduct {
        ...
        public RankedProduct(int rank, Product product, Stock stock, int orderCount) {
            this.rank = rank;
